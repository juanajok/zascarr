...
- **B4 (parcial, Comic Vine + AniList):** `EnrichmentService` enruta cada serie a UNA fuente según `Series.tradition`: `american`/`british` → Comic Vine, `manga`/`manhwa`/`manhua` → AniList (`src/secuenciarr/services/anilist.py`, GraphQL público, sin API key). BD/tebeo/fumetti se dejan sin tocar a propósito — ninguna de las dos fuentes las indexa bien. AniList enriquece solo a nivel de Serie (título/sinopsis/portada/nº capítulos vía `chapters`): no tiene un concepto de "issue" equivalente al de Comic Vine, así que `Issue.metadata_source` nunca se pone a `anilist`. Nueva columna `series.anilist_id` (migración `0003`), paralela a `comic_vine_id`. GCD y Tebeosfera quedan pendientes — el diseño sigue siendo pluggable, añadirlas no requiere tocar el core del enricher.
