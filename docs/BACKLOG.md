...
- **Bug real de producción, reportado y diagnosticado con datos reales del usuario (2026-09-25, post-release)**: Prowlarr/Transmission/aMule fallaban con "no se pudo conectar" en una instalación real, con URL/credenciales correctas. Primera hipótesis (servicio atado a `127.0.0.1`) **descartada con evidencia real**: `ss -tlnp` en la Pi del usuario mostró los tres escuchando en `0.0.0.0`/`*`. Causa real, confirmada con `sudo ufw status` + `ip addr show docker0`: un cortafuegos `ufw` con política `DROP` y reglas "solo LAN" (`192.168.1.0/24`, etc.) para esos puertos — el puente de Docker (`172.17.0.0/16` en ese caso concreto) no encajaba en ninguna regla permitida, así que ufw descartaba la conexión antes de llegar al servicio. No es un bug de ZascArr: es un contrato de red que el instalador no documentaba. Mensaje de "Probar conexión" corregido para cubrir ambas causas (ufw primero, bind después); README ampliado con el comando exacto de `ufw allow` para la subred del puente Docker. Lección del propio proceso: una hipótesis bien razonada pero no verificada con datos reales del usuario puede ser sencillamente incorrecta — hubo que pedir el diagnóstico exacto y corregir en base a la evidencia, no a la plausibilidad.
