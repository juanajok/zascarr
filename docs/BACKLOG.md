...
| B11 | Como coleccionista que ya tiene su tebeoteca organizada en disco (el caso normal al instalar por primera vez, no uno raro), quiero que ZascArr la reconozca sola en el primer arranque, sin tener que redescargar ni mover nada — como hace Sonarr al añadir una carpeta raíz con series que ya existen | `Importer` hoy SOLO escanea `transmission_download_dir`/`amule_incoming_dir`/`downloads_path`; `library_path` es puramente destino de escritura (`build_library_path`), nunca origen de lectura — confirmado por código, no supuesto. Nuevo modo "adoptar biblioteca": escanea `library_path` recursivamente, corre triage+matcher igual que el importador normal, pero **registra en BD sin mover ni renombrar** (los archivos ya están donde el coleccionista los puso — mover/renombrar aquí sería el mismo error que ya evita H5 de A3, solo que sobre la biblioteca en vez de sobre la instalación). Dispara sola en el primer arranque si `library_path` tiene archivos y la tabla `series` está vacía (equivalente al "Import Existing Series" de Sonarr al añadir una root folder) — visible en el dashboard como un ciclo más de `import_runs`, no bloqueante (E1 ya deja precedente: nada crítico se congela por un escaneo largo) | P1 | L |
| B12 | Como coleccionista, cuando el parser SÍ extrae título+número pero no hay serie que iguale (o el score queda por debajo del umbral), quiero ver una sugerencia con la que confío en un clic, en vez de rebuscar a mano en Pendientes | Fase 2 del ampliado de `naming.py`/`matcher.py` de 2026-09-25: `MatchResult` ya trae `candidates`/`notes` en el caso `unsorted` — falta superficie de UI en `/ui/pendientes` que muestre el mejor candidato con su score y un botón "sí es esta serie" (confirmación explícita, nunca autoasignación). Requiere C0 para tener series que ofrecer como candidato en una biblioteca nueva | P1 | M |
| B13 | Como coleccionista, si asigno a mano varios archivos del mismo patrón ("La Patrulla X Omnigold N (...)") a la misma serie, quiero que ZascArr deje de preguntarme para ese patrón | Fase 3: tabla de alias/reglas locales (patrón de nombre → series_id), aprendida de las asignaciones manuales de Pendientes/B2, consultada ANTES del matcher fuzzy. Alias local, nunca una regla global que pueda afectar a otra biblioteca — coherente con "el sistema gestiona, nunca facilita" (CLAUDE.md §5) | P2 | M |
...
| ID | Historia | Aceptación | P | Est |
|---|---|---|---|---|
| C0 | Como coleccionista, quiero buscar una serie que NO tengo todavía (por título, en fuentes externas) y darla de alta en un clic, en vez de necesitar que ya exista una `Series` local | Desde Biblioteca o Wishlist, un cuadro de búsqueda consulta Tebeosfera/Comic Vine/AniList (mismas fuentes que ya usa el enricher, B4), muestra resultados con tradición/editorial/año/portada, y al elegir uno crea la `Series` local (con esos metadatos ya rellenos) — opcionalmente añadida directo a Deseados. Bug de producto real, reportado: D1 (wishlist) y el matcher (naming.py/matcher.py) SOLO trabajan contra series que YA existen en la BD; en una instalación nueva (`series` vacía) escribir un título en Deseados no encuentra nada que buscar, y el importador manda todo a `_Unsorted` aunque el nombre se parseara perfecto — no es un fallo de ninguno de los dos, es que falta el paso de alta que los alimenta a ambos | P0 | L |
...
| D11 | Como coleccionista, quiero configurar Prowlarr/Transmission/aMule desde la UI con un botón de "probar conexión", en vez de editar `.env` a mano | Pantalla de ajustes con los campos que hoy solo existen en `.env`/`config.py`, desactivados por defecto; validación en vivo contra cada servicio con estado visible (desactivada/no configurada/disponible/degradada/error); secretos nunca se devuelven ni se muestran completos tras guardarse; migración desde `.env` documentada, sin romper instalaciones existentes; configurar una integración no habilita búsquedas por sí sola (la aceptación legal sigue siendo independiente) | P1 | M |
